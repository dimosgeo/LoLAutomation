...
from utils import Lanes, lane_indexes, colors, fonts, Spacing
...
class Build(tk.Frame):
	def __init__(self, parent: tk.Misc, rune_icons: List[bytes], lane_navigation_icons: Dict[str, Dict[str, str]], swap_spell_function, set_skin_function, set_client_data_func, vertical_space: int = 20, horizontal_space: int = 20, ability_cell_size: int = 34, champion_frame_size: int = 110, item_size: float = 64, *args, **kwargs) -> None:
		tk.Frame.__init__(self, parent, background=colors['background'], *args, **kwargs)
		self.parent = parent
		self.champion = None
		self.rune_sheet = RuneSheet(self, rune_icons, vertical_space=15)
		self.lane_navigation = ControlFrame(self, lane_navigation=lane_navigation_icons, width=self.rune_sheet.width, func=self.select_lane, background=self['bg'])
		self.spacing = Spacing(vertical=vertical_space, horizontal=horizontal_space)
		self.abilities = AbilitiesTable(self, cell_size=ability_cell_size, border_size=2)
		...
		right_column_width = self.width - self.rune_sheet.width - self.spacing.horizontal * 3
		self.starting_items = ItemBuildFrame(self, title='Starting Items', width=right_column_width, height=item_size, title_width=150, background=self['bg'])
		self.full_build = ItemBuildFrame(self, title='Full Build', width=right_column_width, height=item_size, title_width=150, background=self['bg'])
		self.set_client_data_function = set_client_data_func
		self.height = int(self.champion_frame.height + self.lane_navigation.height + self.rune_sheet.height + vertical_space)
		self.skins = SkinChooser(self, width=right_column_width, height=self.height - self.champion_frame.height - self.abilities.height - self.starting_items.height - self.full_build.height - 5 * self.spacing.vertical, pick_skin=set_skin_function)
		self.previous_selected = ''

	def set_champion(self, champion: Dict[str, Any]) -> None:
		self.champion = champion
		self.lane_navigation.set_lanes(champion['build'].keys())
		self.champion_frame.set_champion(champion['name'], champion['image'])
		self.abilities.set_abilities(champion['abilities'])
		self.select_lane(Lanes(champion['default_lane']))

	def set_skins(self, skins) -> None:
		self.skins.set_skins(skins)

	def select_lane(self, lane: Lanes) -> None:
		...
		self.lane_navigation.set_values(self.champion['build'][lane][0]['wr'], self.champion['build'][lane][0]['br'], self.champion['build'][lane][0]['pr'])
		self.lane_navigation.select_lane(lane)
		self.previous_selected = lane

		self.set_client_data_function(self.champion['build'][lane][1])
	...
	def place(self, x=0, y=0) -> None:
		...
		self.lane_navigation.place(x=int(self.spacing.horizontal), y=y)
		y += self.lane_navigation.height
	...
	def place_forget(self) -> None:
		...
		self.lane_navigation.place_forget()
...
class ControlFrame(tk.Frame):
	def __init__(self, parent, lane_navigation: Dict[str, Dict[str, str]], width: float, horizontal_spacing: float = 20, func: callable = lambda *args: None, *args, **kwargs) -> None:
...
class LaneFrame(tk.Frame):
	def __init__(self, parent, lane_navigation, height: float, horizontal_spacing=5, func: callable = lambda *args: None, *args, **kwargs) -> None:
		...
		self.width = 0
		self.horizontal_spacing = horizontal_spacing
		self.button_list = [LaneButton(self, lane=lane, active_icon=lane_navigation[lane]['enabled'], inactive_icon=lane_navigation[lane]['disabled'], size=int(self.height), borderwidth=0, background=self['bg'], activebackground=self['bg'], func=self.toggle_lane) for lane in Lanes if lane in lane_navigation]
		self.active_buttons = []
	...
	def select_lane(self, lane: int) -> None:
		...
		self.button_list[lane_indexes[lane]].set_active(True)
		self.button_list[lane_indexes[lane]]['bg'] = colors['widget_highlight']
		self.previous_selected = lane_indexes[lane]

	def toggle_lane(self, lane: int) -> None:
		if self.previous_selected == lane_indexes[lane]:
			return
		self.lane_func(lane)
	...
	def set_active_buttons(self, lanes) -> None:
		self.active_buttons = []
		for lane in lanes:
			if lane in lane_indexes:
				self.active_buttons.append(lane_indexes[lane])
...
class LaneButton(tk.Button):
	def __init__(self, parent, active_icon, inactive_icon, size: int, lane: Lanes, func: callable = lambda *args: None, *args, **kwargs):
