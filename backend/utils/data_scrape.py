...
async def load_page(session, queue_name='5v5', champion_name='aatrox', lane='top'):
...
def load_pages(queue_name='5v5', champion_name='aatrox'):
	if platform.system() == 'Windows':
		asyncio.set_event_loop_policy(asyncio.WindowsSelectorEventLoopPolicy())
	data = asyncio.run(create_page_tasks(queue_name, champion_name))
	result = dict()
...
async def create_page_tasks(queue_name='5v5', champion_name='aatrox'):
	async with aiohttp.ClientSession() as session:
		tasks = [asyncio.create_task(load_page(session, queue_name, champion_name, lane.value)) for lane in Lanes]
